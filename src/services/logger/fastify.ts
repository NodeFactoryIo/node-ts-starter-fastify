...
function format(
  req: FastifyRequest<Record<string, unknown>, RawServerDefault>
): { msg: string } {
  return {
    msg: `${req.ip} -> ${req.hostname}\t${req.method}:${req.url}\tRequestId: ${req.id}`,
  };
}
...
class FastifyLogger implements FastifyLoggerOptions {
  ...
  private transform = (
    ...
    if (log.stack) {
      callback(null, {
        level: "error",
        message: `RequestId: ${log.reqId} ${log.stack}`,
      });
    } else if (log.req) {
